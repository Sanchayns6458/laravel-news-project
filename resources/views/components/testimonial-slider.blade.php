<article>
    <div class="entry-container">
        <div class="entry-header">
            <h2 class="entry-title">{{{$title}}}</h2>
        </div>

        <svg viewBox="0 0 508.044 508.044">
            <path
                d="M507.93,155.673c0-0.055,0.006-0.11,0.006-0.165c0-66.793-54.145-120.938-120.938-120.938S266.061,88.714,266.061,155.508
        c0,66.794,54.15,120.938,120.938,120.938c13.727,0,26.867-2.393,39.162-6.609c-27.209,156.09-148.93,256.752-36.096,173.905
        C515.182,351.874,508.07,159.198,507.93,155.673z">
            </path>
            <path
                d="M120.938,276.445c13.727,0,26.867-2.393,39.168-6.609c-27.216,156.09-148.937,256.752-36.102,173.905
        c125.117-91.867,118.006-284.543,117.865-288.068c0-0.055,0.006-0.11,0.006-0.165c0-66.793-54.144-120.938-120.937-120.938
        C54.144,34.57,0,88.714,0,155.508C0,222.302,54.15,276.445,120.938,276.445z">
            </path>
        </svg>

        <div class="entry-meta">
            <span class="posted-on">
                <span class="screen-reader-text">Posted on</span>
                <a href="#" rel="bookmark" tabindex="0"><time class="entry-date published"
                        datetime="2018-10-24T08:24:51+00:00">{{$date}}</time><time class="updated"
                        datetime="2018-10-26T11:02:04+00:00">{{$date}}</time></a>
            </span><!-- .posted-on -->
            <span class="testimonial-name"><a href="">{{$postedBy}}</a></span>
        </div>
    </div>
</article>